// lib/hr/views/job_applicants/commission_applicants_page.dart
import 'dart:convert';
...
class CommissionApplicantsPage extends StatefulWidget {
  final int jobId;
  final String? jobTitle;

  const CommissionApplicantsPage({
    super.key,
    required this.jobId,
    this.jobTitle,
  });
  ...
  @override
  State<CommissionApplicantsPage> createState() =>
      _CommissionApplicantsPageState();
}

class _CommissionApplicantsPageState extends State<CommissionApplicantsPage> {
  bool _loading = true;
  String? _error;
  List<Map<String, dynamic>> _applicants = [];

  // per-row loading flags (shows spinner for only the row being updated)
  final Set<int> _rowsLoading = {};

  // commission-specific endpoints
  final String _fetchUrl =
      'https://dialfirst.in/quantapixel/badhyata/api/commissionBasedApplicantsByJob';
  final String _updateHrApprovalUrl =
      'https://dialfirst.in/quantapixel/badhyata/api/updateHrApprovalCommissionBased';
  ...
  /// Update a single applicant's hr_approval (1=approve, 3=reject)
  Future<bool> _updateHrApprovalRow({
    ...
    final oldVal = _applicants[index]['hr_approval'];

    // optimistic update
    setState(() => _applicants[index]['hr_approval'] = newValue);

    try {
      final res = await http.post(
        Uri.parse(_updateHrApprovalUrl),
        headers: {'Content-Type': 'application/json'},
        body: jsonEncode({"id": recordId, "hr_approval": newValue}),
      );
      ...
      if (res.statusCode == 200) {
        ...
        if (ok) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text(body['message'] ?? 'HR approval updated'),
              behavior: SnackBarBehavior.floating,
              action: SnackBarAction(
                label: 'Undo',
                onPressed: () {
                  // local revert only (if you need server undo, call API)
                  setState(() => _applicants[index]['hr_approval'] = oldVal);
                },
              ),
            ),
          );
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text(body['message'] ?? 'Failed to update'),
              behavior: SnackBarBehavior.floating,
            ),
          );
          return false;
  ...
  // helpers (copied/kept consistent with salary UI)
  ...
  int _recordIdFromApplicant(Map<String, dynamic> a) {
    final cand = a['id'] ?? a['applicant_id'] ?? a['applicantId'];
                    ...
                    child: _applicants.isEmpty
                        ...
                        : ListView.separated(
                            ...
                            itemBuilder: (context, i) {
                              ...
                              final int recordId = _recordIdFromApplicant(a);
                                                    ...
                                                    child: Padding(
                                                      ...
                                                      child: Row(
                                                        ...
                                                        children: [
                                                          ...
                                                          // details column (name, contact, applied, remarks)
                                                          Expanded(
                                                            child: Column(
                                                              crossAxisAlignment:
                                                                  CrossAxisAlignment
                                                              ...
                                                              onPressed:
                                                                  ...
                                                                  : () async {
                                                                      final confirm = await _confirm(
                                                                        context,
                                                                        'Approve applicant?',
                                                                        'Approve ${name.isEmpty ? 'this applicant' : name}?',
                                                                        'Approve',
                                                                      );
                                                                      if (confirm !=
                                                                          true)
                                                                        return;
                                                                      await _updateHrApprovalRow(
                                                                        recordId:
                                                                            recordId,
                                                                        newValue:
                                                                            1,
                                                              ...
                                                              onPressed:
                                                                  ...
                                                                  : () async {
                                                                      final confirm = await _confirm(
                                                                        context,
                                                                        'Reject applicant?',
                                                                        'Reject ${name.isEmpty ? 'this applicant' : name}?',
                                                                        'Reject',
                                                                      );
                                                                      if (confirm !=
                                                                          true)
...
/// Simple placeholder profile page - reuse / adapt your real profile screen here.
  ...
  Widget build(BuildContext context) {
    final name =
        ((applicant['first_name'] ?? '') + ' ' + (applicant['last_name'] ?? ''))
            .trim();
    final img = (applicant['profile_image'] ?? '').toString();
    return Scaffold(
      appBar: AppBar(
          ...
          children: [
            Center(
              child: CircleAvatar(
                ...
                backgroundImage: img.toLowerCase().startsWith('http')
                    ? NetworkImage(img)
                    : null,
                child: (img.isEmpty)
                    ? Text(
