...
class HrEmployeeQueryPage extends StatefulWidget {
  ...
  @override
  State<HrEmployeeQueryPage> createState() => _HrEmployeeQueryPageState();
}

class _HrEmployeeQueryPageState extends State<HrEmployeeQueryPage> {
  ...
  Future<void> _fetchQueries() async {
    ...
    try {
      final uri = Uri.parse("${ApiConstants.baseUrl}HrQueryList");
  ...
  Future<void> _sendReply(int id) async {
    ...
    if (text.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(behavior: SnackBarBehavior.floating, content: Text("Please write a reply")),
      );
      return;
    }
    ...
    try {
      final uri = Uri.parse("${ApiConstants.baseUrl}HrQueryReply");
      final body = {
        "id": id,
        "reply_message": text,
        "status": "replied",
      ...
      if (res.statusCode == 200) {
        ...
        setState(() {
          final idx = _queries.indexWhere((q) => (q['id'] as int) == id);
          if (idx != -1) {
            _queries[idx] = {
              ..._queries[idx],
              "reply_message": text,
        ...
        if (!mounted) return;
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(behavior: SnackBarBehavior.floating, content: Text("Reply submitted successfully")),
        );
      } else {
        if (!mounted) return;
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(behavior: SnackBarBehavior.floating, content: Text("Error ${res.statusCode}: failed to reply")),
        );
      }
    } catch (e) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(behavior: SnackBarBehavior.floating, content: Text("Network error: $e")),
      );
    } finally {
                          ...
                          : RefreshIndicator(
                              ...
                              child: ListView.builder(
                                ...
                                itemBuilder: (context, index) {
                                  ...
                                  final reply = (q['reply_message'] ?? '').toString();
                                  final status = (q['status'] ?? 'open').toString();
                                  final employerId = q['employer_id']?.toString() ?? '—';
