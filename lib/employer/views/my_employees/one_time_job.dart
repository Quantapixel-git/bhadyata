...
class OneTimeEmployeesPage extends StatefulWidget {
  const OneTimeEmployeesPage({super.key});

  @override
  State<OneTimeEmployeesPage> createState() => _OneTimeEmployeesPageState();
}

class _OneTimeEmployeesPageState extends State<OneTimeEmployeesPage> {
  bool _loading = true;
  bool _error = false;
  ...
  Future<void> _fetchEmployees() async {
    ...
    try {
      ...
      final uri = Uri.parse('$apiBase/oneTimeEmployeesByEmployer');
    ...
    if (isWeb) {
      return GridView.builder(
        ...
        itemBuilder: (context, idx) {
          ...
          return _OneTimeEmployeeCard(employee: e);
        },
      );
    } else {
...
/// Card for one-time job employee — shows payment fields
class _OneTimeEmployeeCard extends StatefulWidget {
  final Map<String, dynamic> employee;
  const _OneTimeEmployeeCard({required this.employee});

  @override
  State<_OneTimeEmployeeCard> createState() => _OneTimeEmployeeCardState();
}

class _OneTimeEmployeeCardState extends State<_OneTimeEmployeeCard> {
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    // primary assignment summary (one-time specific)
                    if (primary != null)
                      Row(
                        children: [
                          Flexible(
                    ...
                    if (primary != null)
                      Text(
                        '${primary['payment_amount'] ?? '0.00'} • ${primary['payment_type'] ?? ''}',
                        style: const TextStyle(
                          fontSize: 12,
                          color: Colors.black45,
                        ),
                              ...
                              Expanded(
                                child: Column(
                                  ...
                                  children: [
                                    ...
                                    const SizedBox(height: 4),
                                    Text(
                                      'Joined: ${_fmtDate(asg['joining_date'])} • Amount: ${asg['payment_amount'] ?? '0.00'}',
                                      style: const TextStyle(
                                        color: Colors.black54,
                                        fontSize: 12,
                                      ),
