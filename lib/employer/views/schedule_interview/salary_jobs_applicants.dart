...
class _EmployerSalaryBasedJobsWithApplicantsPageState
  ...
  final String apiUrl =
      '${ApiConstants.baseUrl}salaryBasedjobsWithApplicants';
    ...
    try {
      ...
      if (res.statusCode == 200) {
        ...
        if (json['success'] == true) {
          ...
          setState(() => _error = (json['message'] ?? 'Failed to load').toString());
        }
      } else {
                            ...
                            : ListView.builder(
                                ...
                                itemBuilder: (context, i) {
                                  ...
                                  final count = (j['applicants_count'] is int)
                                      ...
                                      : int.tryParse(_safe(j['applicants_count'])) ?? 0;

                                  // Salary-based friendly bits (optional, if present)
                                  final jobType = _safe(j['job_type']); // may not exist
                                  final salaryMin = _safe(j['salary_min']); // may not exist
                                  final salaryMax = _safe(j['salary_max']); // may not exist

                                  // If salary fields are missing, we won’t show salary line.
                                  final hasSalaryRange = salaryMin != '—' || salaryMax != '—';
